...
class Logger {
    ...
    static initialize(client) {
        if (false === (client instanceof MQTTClient))
            throw new Error('The client parameter must be an instance of MQTTClient.');
    ...
    static log(message, topic = '') {
        if (false === (this.#instance instanceof Logger)) {
            ...
            if (topic && false === topic.startsWith(String.fromCharCode(47)))
                topic = String.fromCharCode(47) + topic;
